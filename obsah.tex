...
Tato práce se pokusí pøispìt svým dílem navr¾ením nové metody ovládání leteckých her pro mobilní zaøízení. Nejdøíve bude popsána platforma na které bude implementace probíhat, dále budou vysvìtleny principy, díky kterým letí a manévruje skuteèné letadlo. Tyto principy budou poté zjednodu¹eny pro pou¾ití v~prostøedí her. Dále budou shrnuty souèasné mo¾nosti ovládání herních letadel a navr¾ena nová metoda jejich ovládání, zalo¾ená na dotyku. Nad touto metodou bude vystavìna plnohodnotná letecká hra, její¾ vývoj bude popsán od prvotního návrhu a¾ po její implementaci a publikování v~internetovém obchodì Google Play. Souèástí práce bude také prùzkum reakcí hráèù a jeho zhodnocení. Na závìr bude nabídnuta také kapitola shrnující dal¹í mo¾ný vývoj této hry.
...
Vývoj operaèního systému vede firma Google, která vydává zdrojové kódy nových verzí. Ty se ov¹em pøímo do zaøízení dostávají jen velmi zøídka (jde o~referenèní modely telefonù a tabletù které si nechává vyrábìt pøímo Google). Typicky pøebírají kód jednotliví výrobci a upravují èi doplòují jej tak, aby fungoval pro konkrétní modely (nekompatibilita hrozí pøedev¹ím u~specifického hardwaru jako je 3D displej a dal¹í). Èasto také výrobce pøidává dodateèné aplikace, a» u¾ pro plné vyu¾ití mo¾ností zaøízení nebo èistì pro reklamní úèely. Jak bude výsledný systém vypadat, jaké funkce bude obsahovat a jaká zaøízení bude podporovat je plnì v~zodpovìdnosti výrobcù. Právì kvùli tomu jsou nejèastìji aktualizovány modely z~vy¹¹í cenové tøídy a levnìj¹í zaøízení bývají opomíjena.

Dùsledkem otevøenosti Androidu je jeho roztøí¹tìnost co se verzí týèe. Aktuální rozlo¾ení verzí \textit{API} (programátorské rozhraní, Application Programming Interface) k~dubnu 2013 je na obrázku \ref{fig:api-verze}, data pocházejí z~oficiálního zdroje \cite{statsDroid} a jsou mìøena podle toho, jak se zaøízení s~danými verzemi pøipojují k~obchodu Google Play. Statistika tedy nezobrazuje podíly verzí prodaných zaøízení, nýbr¾ tìch, která alespoò jednou nav¹tívila oficiální obchod s~aplikacemi. Z~hlediska vývojáøe má tato metodika mnohem vy¹¹í vypovídací schopnost. Je vidìt, ¾e více ne¾ tøetinový podíl má verze 10 (v~u¾ivatelském znaèení jde o~Android verze 2.3.x) která je i nyní, po témìø dvou letech od prvního vydání, stále nasazována do novì prodávaných zaøízení. Jde v¹ak pøedev¹ím o~zaøízení nejni¾¹í cenové a výkonnostní tøídy.

\begin{figure}[ht]
	\begin{center}
		\vspace{\baselineskip}
		\includegraphics[width=\textwidth]{fig/api-versions.pdf}
		\caption{Rozlo¾ení verzí systému Android k~dubnu 2013. Obecné aplikace musejí øe¹it kompatiblitu s~co nevìt¹ím mno¾stvím verzí systému. Hry v¹ak pou¾ívají pouze minimum systémových funkcí, a lze tak èasto pokrýt více ne¾ 94\% zaøízení. Zbývající procenta pøedstavují nejménì výkonná zaøízení s~nejstar¹ími verzemi systému. Zdroj dat: oficiální portál Android \cite{statsDroid}}
		\label{fig:api-verze}
	\end{center}
\end{figure}
...
V~pøípadì ¾e zvolíme tvorbu hry èistì na základì Androidu bez mezièlánkù typu frameworku nebo herního engine, mù¾eme narazit na nìkteré nepøíjemnosti. Pøedev¹ím nemù¾eme poèítat s~jakýmkoliv roz¹íøením nad mo¾nosti OpenGL ES. Je plnì v~kompetenci programátora aby si pøipravil potøebné datové struktury a pomocné tøídy. Systém jako takový sice obsahuje nìkolik základních tøíd pro popis vektorù, matic a operace s~nimi, ty ale nebyly navr¾eny pro práci s~OpenGL a dochází zde k~nesrovnalostem (pøíkladem mù¾e být vnitøní reprezentace matice, která je jednou øádková a jindy sloupcová). Na vìt¹inu tìchto problémù poukazuje autor knihy o~vývoji her pro Android \cite{libgdxKniha}, který je souèasnì autorem knihovny Libgdx, která tyto nedostatky napravuje.
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Hra Shadowgun od brnìnského studia Madfinger Games, vytvoøená v~prostøedí Unity 3D, zdroj: \url{www.madfingergames.com}}
		\label{fig:shadowgun}
	\end{center}
\end{figure}
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Souèásti rozhraní hlavního menu hry. Èervenì jsou orámovány hlavní komponenty, modøe kontejnery a zelenou barvou je vyznaèeno odsazení vnoøené komponenty od okraje svého rodièe.}
		\label{fig:ui-design}
	\end{center}
\end{figure}
...
Nezbytnou souèástí zejména leteckých her bude jistá forma fyzikální simulace. K~tomuto úèelu popí¹i volnì dostupnou knihovnu Bullet Physics. Ta mù¾e fungovat v~nìkolika re¾imech, pøièem¾ dva nejvýznamnìj¹í jsou re¾imy tzv. \textit{kolizního svìta} (tøída \texttt{btCollisionWorld}) a \textit{simulaèního svìta} (tøída \texttt{btDiscreteDynamicsWorld}). Základní principy jednoduché 2D a 3D simulace popisuje ji¾ zmiòovaná kniha o~vývoji her \cite{libgdxKniha}, informace specifické pro knihovnu Bullet Physics pak vycházejí z~její oficiální webové stránky \cite{bullet} (manuál ve formátu PDF je souèástí sta¾itelného vývojáøského balíèku).

Jak ji¾ název napovídá, kolizní svìt obsahuje entity reprezentované svými kolizními tvary a signalizuje, pokud dojde k~jejich kontaktu. Na tento kontakt lze následnì reagovat v~rámci herní logiky. Entity se ale dále mohou pohybovat a v~kolizi pokraèovat. Knihovna zde slou¾í èistì k~detekci kolizí a nikoliv k~jejich dal¹í obsluze. Detekce probíhá ve dvou fázích -- tzv. \textit{¹iroké} a \textit{úzké}. Pøi ¹iroké fázi jsou namísto skuteèných obalových tìles porovnávány kolize osovì zarovnaných obalových kvádrù (metoda \textit{AABB - Axis Aligned Bounding Boxes}). Tímto krokem je mo¾né rychle odstranit velké mno¾ství dvojic objektù, které spolu nemohou v~¾ádném pøípadì kolidovat. V~úzké fázi se následnì hledá kolize mezi skuteènými tvary entit. Tento krok je znaènì nároènìj¹í, nebo» obalové tvary mù¾ou být jednoduché (základní tvary jako koule, kvádr, plocha), ale také slo¾ené z~obecnì libovolného mno¾ství základních a slo¾ených tvarù. Se slo¾itostí tvaru pøirozenì roste i nároènost ovìøení kolize. Z~tohoto dùvodu je zavedeno rozdìlení do dvou fází, které toto slo¾ité porovnání provede jen v~pøípadech, kde mù¾e kolize reálnì nastat.

Nad kolizním svìtem je vystavìný svìt simulaèní. V~tomto svìtì obsahují entity více informací. Kromì kolizního tvaru je nutná znalost hmotnosti, rychlosti a zrychlení v~èase a volitelnì dal¹ích vlastností jako napøíklad tvrdost èi pru¾nost povrchu. Tyto entity jsou pak souèástí plnohodnotné fyzikální simulace, která sama øídí jejich pohyb a pøípadné kolize. V~praxi funguje zapojení simulace do hry nejèastìji tak, ¾e herní logika dodává impulsy (napøíklad sílu vzniklou vysunutím klapky letadla) a o~zbytek se stará právì fyzikální knihovna.

Provádìní úplné fyzikální simulace je ve srovnání s~pouhou detekcí kolizí øádovì slo¾itìj¹í. Dokonce natolik, ¾e se v~souèasných hrách pro mobilní zaøízení pou¾ívá jen minimálnì, pøípadnì jen pro velmi malé mno¾ství entit. Takto lze dosáhnout dobrého dojmu ze hry, kdy hráè vnímá hru jako reálnou, pøesto¾e fyzikální simulace probíhá jen u~hlavního objektu a chování ostatních objektù mù¾e být pevnì pøednastaveno. I~v~pøípadì simulace jediného objektu je v¾dy nutné co nejpøesnìji fyzikálnì popsat pùsobící síly i objekt samotný. Souèasnì není mo¾né do této simulace zasahovat a zavádìt vlastnosti, které by realitì neodpovídaly. Ze zmínìných dùvodù plyne, ¾e pro tuto práci bude dále relevantní pouze svìt kolizní.
...
Aplikace pro systém Android lze distribuovat nìkolika rùznými cestami. Tou nejzákladnìj¹í je pøímá distribuce binární podoby aplikace (v~souboru s~pøíponou APK). Tato volba v¹ak znemno¾ní jakoukoliv dal¹í automatickou aktualizaci (manuální aktualizaci lze stále provést získáním nové verze souboru) a souèasnì nedovoluje mít kontrolu nad tím, kteøí u¾ivatelé, na jakých zaøízeních a za jakých podmínek, budou aplikaci pou¾ívat. Navzdory tomu je tato metoda pou¾ívána, napøíklad pri testování prototypu aplikace a jeho kompatibility.

Dùmyslnìj¹ím zpùsobem distribuce je vyu¾ití nìkterého z~internetových obchodù. Tìch existuje hned nìkolik s~rùznì velkou u¾ivatelskou základnou a s~rùznými podmínkami publikování. Dominantní postavení zde má obchod Google Play, pøedstavující oficiální katalog aplikací platformy Android. Jeho hlavní pøedností je, ¾e je pøednastaven v~drtivé vìt¹inì prodávaných zaøízení a má tak mo¾nost oslovit maximální mno¾ství potenciálních u¾ivatelù. Vývojáøi souèasnì nabídne mo¾nost aplikaci prùbì¾nì vzdálenì aktualizovat, øídit která zaøízení bude aplikace podporovat, sledovat statistiky instalací a také sbírat zpìtnou vazbu od u¾ivatelù. Aplikaci zde lze zveøejòovat bezplatnì, ale i za poplatek, pøièem¾ si provozovatel obchodu nárokuje 30\% podíl z~provedené transakce. Zmínìné pøednosti jsou v¹ak vykoupeny jednorázovým poplatkem za registraci vývojáøe, který v~dobì psaní této práce èiní \$25. Dal¹ím nedostatkem obchodu Google Play je nemo¾nost rozdìlit konkrétní u¾ivatele do skupin a tìm následnì nabídnout jinou verzi aplikace nebo napøíklad mo¾nost bezplatného testování placené varianty. Stejnì tak by mnoho vývojáøù ocenilo mo¾nost vygenerování speciálních poukazù, kterými by mohli zdarma zpøístupòovat propagaèní verze svých aplikací.
...
Pøed návrhem herního ovládání letadla je vhodné se seznámit s~tím, jak létají skuteèná letadla. Více ne¾ o~konkrétní rovnice zde pùjde o~základní principy díky kterým se letadlo udr¾uje ve vzduchu. Rovnice by toti¾ nebylo mo¾né vyèíslit bez znalosti konkrétních koeficientù, které jsou pro ka¾dou konstrukci odli¹né. I~s~nimi by v¹ak bylo exaktní matematické popsání letícího letadla velmi obtí¾né. Následující kapitoly budou èerpat pøedev¹ím z~knihy Umìní létat \cite{umeniLetat} a studijních materiálù americké vládní agentury pro letectví (NASA) \cite{nasa}.


\section{Síly pùsobící za letu}

Základní pojem pro dal¹í popis bude takzvaný \textit{ustálený vodorovný let}. V~tuto chvíli se letadlo pohybuje konstantní rychlostí s~nulovým zrychlením, nestoupá, neklesá a není v~náklonu. Pùsobí na nìj síly vyznaèené na obrázku \ref{fig:letadlo-sily}. Jsou jimi vzlak, gravitaèní tíha, tah a odpor. Pøi ustáleném letu jsou tyto síly v~rovnováze. Ne v¹ak v¹echny navzájem, jak je nìkdy mylnì uvádìno, ale pouze dvojice sil pùsobících proti sobì. Pro ilustraci mù¾eme pou¾ít namísto celého letadla pouze køídlový profil, na kterém jsou velièiny více zøejmé.
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Úhel nábìhu køídla. Jeho výchozí hodnota je dána konstrukèními vlastnostmi letadla a køídel. Za letu jej lze pak regulovat vysouváním èi zasouváním klapek a samozøejmì zmìnou orientace celého letadla vùèi smìru proudìní vzduchu.}
...
U~zatáèecího manévru èasto panuje pøesvìdèení, ¾e k~jeho provedení slou¾í pouze smìrové kormidlo, od kterého se u¾ podle názvu oèekává ¾e bude øídit smìr letu. Z~pøedchozích odstavcù ji¾ mù¾eme tu¹it ¾e tomu tak není. Pokud by pilot pøi vodorovném letu pou¾il smìrové kormidlo, záï letadla by se sice vùèi smìru letu natoèila, samotný smìr letu by se v¹ak zmìnil jen málo. Tím jak se osa letadla natáèí vùèi smìru letu souèasnì roste proudìní kolem køídla které je více vystaveno pøímému smìru letu. Tím na køídle vzniká vìt¹í vztlak a letadlo zaèíná podélnì rotovat, aèkoliv je knipl stále ve výchozí poloze.
...
Ka¾dé letadlo má svou konstrukcí dané stabilizaèní schopnosti, které se uplatòují ani¾ by pilot zasahoval do øízení. Stabilitu se dìlí na statickou a dynamickou.
...
Pøi navrhování hry jsem vycházel zejména z~knih Game Industry, které sepsali èe¹tí herní vývojáøi \cite{gameindustry1}, \cite{gameindustry2}.
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Ovládání náklonu letadla. Èervená místa znaèí body dotyku prstù. Intenzita zmìny náklonu odpovídá úhlu, který svírá úseèka ohranièená tìmito body s~vodorovnou osou obrazovky. Body je mo¾né podél vodorovné osy libovolnì posouvat bez vlivu na øízení.}
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Ovládání vzestupu a poklesu letadla. Èervená místa znaèí body dotyku prstù a souèasnì definují úseèku. Intenzita zmìny vý¹ky letadla závisí na vzdálenosti støedu této úseèky od vodorovné osy obrazovky. Body je mo¾né podél vodorovné osy libovolnì posouvat bez vlivu na øízení.}
...
Navrhované ovládání by bylo mo¾né dále roz¹íøit o~mo¾nost ovládání smìrového kormidla. To by pøedstavovalo tøetí ovládací osu, tedy svislou osu kolem které mù¾e letadlo rotovat. Velièinu by bylo mo¾né odeèítat podobnì jako je odeèítána poloha vý¹kového kormidla. ©lo by o~vzdálenost støedu úseèky tvoøené dvìma body dotyku od svislé osy obrazovky, procházející bodem poèáteèního dotyku. (Vý¹kové kormidlo zde mìøí vzdálenost od osy vodorovné.) Vyu¾ití této tøetí ovládací osy v¹ak nemusí být ideální a navrhované ovládání jako celek by mohlo na hráèe pùsobit pøíli¹ slo¾itì. Ideálním øe¹ením by bylo tuto variantu implementovat a získat zpìtnou vazbu od prvních zku¹ebních hráèù.
...
V~této èásti popí¹i implementaci navr¾eného ovládání a hry. Ve¹kerá práce probíhala ve vývojovém prostøedí Eclipse, doplnìné o~moduly specifické pro platformu Android. Tyto doplòky jsou souèástí podkladù pro vývojáøe a jsou dostupné z~oficiálních webových stránek \cite{androidDevelopers}. 

Poslední podkapitola bude vìnovaná publikování vytvoøené hry nejdøíve v~omezené skupinì lidí a následnì na internetovém obchodì Google Play. Zhodnotím také výsledky testování, prùzkumù a poznatky získané sbìrem statistik u¾ívání. V~závìru kapitoly také shrnu mo¾nosti dal¹ího vývoje.
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Jedna z~prvních verzí prototypu vyvíjeného bez pou¾ití pomocných knihoven. Primárním cílem bylo testování ovládání. Vizuální stránka proto byla zpoèátku velmi slabá.}
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Závìreèná podoba prototypu obsahovala nìkolik variant ovládání. I~v~grafickém zpracování nastal posun, nebo» testující hráèe pøíli¹ zjednodu¹ené prostøedí ru¹ilo od hlavního cíle, tedy testování ovládání.}
...
Tato kapitola popí¹e strukturu aplikace, která má v~pøípadì pou¾ití knihovny Libgdx jistá specifika. Budou také pøedstaveny nìkteré diagramy tøíd, zachycující nejdùle¾itìj¹í vazby mezi herními objekty. Úplny diagram tøíd tato práce neobsahuje, nebo» by od ètenáøe vy¾adoval hlubokou znalost pou¾ité knihovny a pro bì¾ného ètenáøe by nebyl pøíli¹ popisný.
...
Øídicí funkci z~hlediska herní logiky má herní obrazovka, tedy instance tøídy \texttt{GameScreen}. Ve chvíli svého vytvoøení dostává v~konstruktoru pøipravený herní svìt (tedy \texttt{GameWorld}) a v~nìm naètený závod. Obrazovka vytvoøí objekt letadla (souèástí definice svìta je pouze jeho poèáteèní transformace), nastaví zpracování vstupù tak, aby se dotykové události dostaly na vstup letadla, nechá probíhat simulaci a souèasnì scénu vykresluje pomocí tøídy \texttt{GameRenderer}. Samotný pohyb letadla ve scénì probíhá velmi jednodu¹e, vyèíslováním vztahu pro rovnomìrný pøímoèarý pohyb, kde jako èasový parametr slou¾í doba uplynulá mezi dvìma vykreslovanými snímky. Díky tomu se bude letadlo pohybovat stále stejnì rychle bez ohledu na to, jaký je aktuální poèet snímkù za sekundu.
...
\begin{figure}[ht!]
	\begin{center}
		...
		\caption{Jednotlivé obrazovky a dialogy v~aplikaci. Pro navigaci mezi nimi slou¾í prvky u¾ivatelského rozhraní nebo volitelnì systémová tlaèítka zaøízení.}
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Dialog pro zauèení nového hráèe, zobrazovaný u~prvního závodu ve høe. Pro spu¹tìní závodu se musí hráè dotknout souèasnì obou ¾lutých kruhù. Tím by mìl pochopit, ¾e k~ovládání letadla je potøeba v¾dy dvou prstù.}
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Rozhraní pro tvorbu a úpravu závodù. Slou¾í pouze pro vývoj a hráèi do nìj pøístup nemají. Ve scénì se lze pohybovat skokovì (tlaèítky) nebo volnì pomocí dotykového vstupu.}
		\label{fig:ui-dev}
	\end{center}
\end{figure}
...
Ve chvíli, kdy se hra dostala do finální podoby, bylo mo¾né ji zaèít veøejnì distribuovat. Souèasnì jsem pro dosud bezejmennou hru zvolil název Air Racer. Do té doby jsem testování provádìl osobními schùzkami v~rùzných stádiích vývoje, kde bylo mo¾né okam¾itì reagovat na chyby a jiné nedostatky. Tento pøístup souèasnì nabízel mnohem lep¹í zpìtnou vazbu od hráèù, kterou by bylo pozdìji obtí¾né získat. Dal¹ím pøínosem bylo také pøímé testování hry na rùzných zaøízeních. Zde ¾ádné problémy nenastaly a hra bez problémù bì¾ela na v¹ech zaøízeních, se kterými deklarovala kompatibilitu. Hodnotné reakce pøinesly také pravidelné konzultace ve firmì CUKETA, s.\,r.\,o.\,, která se herním vývojem profesionálnì zabývá.
...
I~pøes nízkou úèast hráèù v~hodnocení se hra v~obchodì prosadila a necelý mìsíc po vydání se dostala na pozici deváté nejlep¹í závodní hry. Graf vývoje pozice v~jednotlivých dnech je na obrázku \ref{fig:pozice-play}. Celkový poèet instalací pøesahuje 70000, pøièem¾ denní trend nových u¾ivatelù v~souèasnosti spí¹e stagnuje. 

\begin{figure}[ht]
	\begin{center}
		\includegraphics[width=\textwidth]{fig/pozice-play.pdf}
		\caption{Vývoj pozice hry v~Google Play, v~kategorii závodních her. Po necelém mìsíci od vydání je hra na 9. pozici.}
		\label{fig:pozice-play}
	\end{center}
\end{figure}

Souèástí vývojáøského rozhraní obchodu Google Play je i sledování vzniklých výjimek. Tìch se objevilo vzhledem k~mno¾ství instalací jen minimum. ©lo o~nepøedvídatelné chyby, které vznikaly jen na nìkterých jednotlivých modelech nebo konkrétních zaøízeních. Pøesto¾e bývají zaznamenané výjimky bì¾nì svázány s~popisem zaøízení na kterém vznikly, v~nìkterých pøípadech tato informace chybìla. Lze pøedpokládat, ¾e jde o~upravená sestavení platformy Android. Mimo toto jsem narazil také na chybu vyskytující se v~rùzném mno¾ství na celé modelové øadì. Jde o~chybu pøímo v~platformì Android, která se v¹ak vyskytuje jen u~konkrétního sestavení a verze. V~tomto pøípadì bylo mo¾né pøestat tìmto typùm zaøízení hru nabízet a v~obchodì ji oznaèit jako nekompatibilní. V~dobì pøed omezením podpory mìla tato zaøízení témìø 10\% podíl na instalacích. Pøesto¾e jde o~nezanedbatelný poèet hráèù, jiné øe¹ení dostupné nebylo. Chyba byla nahlá¹ena a v~dal¹ích verzích platformy opravena, nicménì výrobce aktualizaci svých pøístrojù nenabídl a jiné øe¹ení ne¾ aktualizace platformy pro tento problém známé není.



\section{Mo¾nosti dal¹ího vývoje}

Pøesto¾e jsem v~døívìj¹ích kapitolách èasto naznaèil dal¹í mo¾ný vývoj, popí¹i jej nyní detailnì v~samostatné kapitole. Za polo¾ku s~nejvy¹¹í prioritou aktuálnì pova¾uji vizuální stránku hry. Tu by sice bylo mo¾né zaøadit v~rámci her nabízených na Google Play do prùmìru, nicménì je to vlastnost, která produkt prodává. Úplnì nejdøíve potenciální zákazník vidí ikonu hry, následnì propagaèní obrázky a teprve poté si mù¾e pøeèíst popis a hru pøípadnì stáhnout. Z~reakcí na hru lze vyèíst, ¾e s~navr¾eným zpùsobem ovládání je vìt¹ina hráèù spokojená, nicménì jde právì o~tu skupinu hráèù, které neodradil ¾ádný ze zmiòovaných krokù. Lze pøedpokládat, ¾e existuje poèetná skupina náv¹tìvníkù internetového obchodu, které hra nezaujala právì kvùli grafickému zpracování. Konverzi tìchto potenciálních hráèù na hráèe skuteèné by mìlo pomoci právì dal¹í zlep¹ování propagaèních materiálù. Jejich souèástí je také popis hry v~obchodì, který by bylo vhodné pøelo¾it do více jazykù (aktuálnì je pouze anglicky). Stejnì tak by bylo mo¾né pøelo¾it v¹echny texty ve høe, jak také napovídaly nìkteré reakce hráèù.

Souèástí vizuální stránky hry je také herní prostøedí. V~tomto pøípadì by úpravy cílily pøevá¾nì na ji¾ existující hráèe. Ti se v~dotazníku svojí vìt¹inou shodli, ¾e by høe prospìlo více dekorativních objektù. Spolu s~tím, jak by v~rámci roz¹iøování pøibývaly ve høe dal¹í závody, by mohly zaèít pùsobit pøíli¹ jednotvárnì. Herní objekty jako tribuny, reklamní plochy a dal¹í by mohly takovou jednotvárnost naru¹it a souèasnì (ov¹em ne nezbytnì) zvý¹it obtí¾nost závodù, napøíklad jejich vhodným umístìním do trasy letu. Ve vìt¹ím mìøítku pak mù¾e jít o~úplnou zmìnu scenerie závodù. Mohly by se odehrávat napøíklad v~pou¹ti, ve mìstì mezi domy anebo témìø v~libovolné krajinì. Bylo by zde nutné dbát na výkonové nároky, které jsou v~souèasnosti velmi nízké, a pøípadná roz¹íøení zavádìt tak, aby nebylo nutné omezovat podporu nìkterých star¹ích a ménì výkonných zaøízení. Z~ménì nároèných úprav pak zmíním napøíklad efekty odlesku a rozptylu svìtla na objektivu kamery (tzv. efekt \textit{lens flare}) nebo viditelné linky vzduchu èi páry u~koncù køídel letadla, které by kromì vizuálního efektu mohly hráèi pomoci lépe si pøedstavit, jakým smìrem se letadlo aktuálnì pohybuje.

\vspace{\baselineskip}

Z~hlediska ovládání zopakuji ji¾ zménìnou mo¾nost ovládání smìrového kormidla. Jeliko¾ by pravdìpodobnì vìt¹ina hráèù tuto zmìnu nepøivítala, bylo by mo¾né takové roz¹íøení nabízet volitelnì. Bylo by v¹ak velmi zajímavé zji¹»ovat dal¹í mo¾nosti navr¾ené metody ovládání. Dùle¾itým aspektem u~úprav tohoto typu v~ji¾ publikované høe by bylo zachování zpìtné kompatibility herních výsledkù. Zmìna ovládání, stejnì jako zmìna parametrù letadla, mù¾e zpøístupnit nové a efektivnìj¹í letové trasy anebo naopak ztí¾it dokonèení ji¾ existujících závodù. Pøípad, kdy hráè dostává mo¾nost dokonèit závod rychleji, je pøirozenì lep¹í variantou. I~ten by v¹ak mohl naru¹it mo¾nost vzájemného srovnávání výkonù hráèù, napøíklad ve sledovaných statistikách.

Podle nìkterých ohlasù by hráèi uvítali také lep¹i pøehled o~konkrétních èasech závodù, napøíklad ve smyslu mo¾nosti zobrazení èasù døívìj¹ích letù. Pokud bych tuto my¹lenku mìl rozvést dál, pravdìpodobnì bych navrhl mo¾nost zobrazení tzv. \textit{ducha}. ©lo by o~èásteènì prùhledné letadlo, které by pøedstavovalo hráèùv døívìj¹í let. Díky tomu by bylo zøejmé, kde se hráè od døívìj¹í trasy odchyluje a namísto pøímého srovnání èasù by hra nabízela srovnání vizuální.

S~mo¾ností vzájemného srovnání souvisí i dal¹í mo¾nost ro¾¹íøení -- online ¾ebøíèky a závody pro více hráèù. Zmínìnou mo¾nost zobrazení duchù by bylo mo¾né roz¹íøit o~sdílení tìchto záznamù závodù mezi hráèi. Ti by pak vedle pøeddefinovaných èasù závodù mohli pøekonávat globální rekordy skutèných hráèù. Toto roz¹íøení by ov¹em znamenalo øádovì více práce, nebo» by bylo nutné zajistit server ukládající tato data a jejich synchronizaci. Nelze také opomenout nutnost zabezpeèení takových pøenosù, aby nebylo mo¾né výsledky závodù podvrhnout.

Jinou cestou roz¹iøování hry pro více hráèù by mohlo být lokální hraní s~vyu¾itím technologií jako Bluetooth nebo Wi-Fi. Zde by nároky na synchronizaci a bezpeènost spojení nebyly tak vysoké a souèasnì by bylo mo¾né nabídnout spoleèné hraní v~reálném èase. To by umo¾nilo také zavedení nových pøístupù k~závodùm, které by vy¾adovaly napøíklad spolupráci obou hráèù nebo naopak jejich vzájemné soupeøení, v~extrémním pøípadì i formou vzdu¹ných soubojù (které hráèi v~dotazníku také nìkolikrát zmínili).

Pro úplnost zmíním také pøidávání dal¹ích závodù, pøesto¾e jde o~roz¹íøení naprosto zøejmé. Tímto zpùsobem by bylo mo¾né udr¾ovat v~aktivitì existující hráèe. Opìt zde ale hrozí jistá monotónnost a bylo by vhodné tato roz¹íøení kombinovat právì se zmínìnými grafickými doplòky. Zajímavým, by» opìt realizaènì nároèným roz¹íøením, by byla mo¾nost vytváøení vlastních závodù samotnými hráèi. Základní prvky pro tuto funkcionalitu u¾ hra obsahuje, popsány byly v~kapitole \ref{sec:vyvojarske-rozhrani}. Po nezbytných úpravách by se tato dosud vývojáøská obrazovka mohla stát plnohodnotnou souèástí hry. Hráèi by mohli vytvoøené závody sdílet a soupeøit o~nejlep¹í èasy, a» u¾ globálnì èi lokálnì. Opìt by ale platily døíve uvedené pøedpoklady o~zabezpeèení takovéto funkcionality proti zneu¾ití.

\vspace{\baselineskip}

Pøi úvahách o~dal¹ím roz¹iøování hry je vhodné uvá¾it také ekonomickou stránku vìci. Zatímco nìkteré z~navrhovaných úprav jsou jednoduché, jiné by vy¾adovaly napøíklad provoz webového serveru. V~takovou chvíli by nutnì vznikaly náklady, které by vzhledem k~poètu hráèù zøejmì nebyly nízké. Pøesto¾e náklady na dosavadní vývoj hry pøedstavovaly pouze strávený èas, i tento pøedstavuje omezené prostøedky, které by mìly vést k~zisku (by» ne nezbytnì materiálnímu). U~her pro mobilní zaøízení bývá obecným zvykem vydat hru ve dvou variantách -- bezplatné a zpoplatnìné. Bezplatná verze hry mù¾e slou¾it jako demo a souèasnì èasto obsahuje omezenou nabídku funkcí. V~tomto pøípadì by bylo mo¾né vydat hru obohacenou o~nìkteré z~vý¹e uvedených roz¹íøení ve zpoplatnìné verzi. Pøesto¾e by si placenou verzi zakoupil zøejmì jen zlomek stávajících hráèù, bylo by mo¾né uva¾ovat o~alespoò èásteènému návratu investic do vývoje èi provozu hry. Souèasnì by ale vznikaly dal¹í problémy, kterými bezplatná verze netrpí, jako napøíklad pirátství. To mù¾e u~¾ádaných her mù¾e dosahovat a¾ k~90\% nelegálnì získaných kopií \cite{pirati}. Dal¹í výzvou by tedy bylo nastavení vhodného obchodního modelu tak, aby se tyto ztráty minimalizovaly. Pøíkladem takového modelu mù¾e být nákup pøímo v~aplikaci (tzv. \textit{in-app payments} nebo \textit{in-app transactions}), kdy se samotná aplikace se základní funkcionalitou distribuuje bezplatnì a dokupování dal¹ího obsahu a mo¾ností probíhá pøímo v~aplikaci.
...
Tento text shrnul mo¾nosti vývoje 3D her na platformì Android s~vyu¾itím knihovny Libgdx. Dále se vìnoval popisu letu skuteèného letadla a z~nìj vycházející zjednodu¹ené simulace letu pou¾ívané ve hrách. Srovnává také existující metody ovládání leteckých her a navrhuje novou metodu, která záva¾nými nedostatky oproti ostatním netrpí. Navrhovaná metoda je zalo¾ená na dotykovém vstupu za pou¾ití dvou prstù a nabízí velmi rychlé a pøesné ovládání, èím¾ posiluje dojem, ¾e má hráè letadlo plnì pod kontrolou.

Dále tato práce popsala návrh a implementaci hry, která toto ovládání vyu¾ívá. Jde o~hru, kde hráè s~akrobatickým letadlem prolétává brankami a sna¾í se doletìt do cíle v~co nejkrat¹ím èase. Hra je navr¾ená tak, aby co nejvíce tì¾ila z~pøedstavené ovládací metody, podpoøila tak její my¹lenku a v~ideálním pøípadì i dal¹í roz¹íøení na poli mobilních leteckých her. V~prùbìhu vývoje vznikl prototyp hry, který ovìøil základní principy ovládání a umo¾nil srovnání s~ji¾ pou¾ívanými metodami. Následnì vznikla plnohodnotná implementace hry s~vyu¾itím právì knihovny Libgdx. 

Hra byla prùbì¾nì konzultována s~firmou CUKETA, s.\,r.\,o.\,, která se vývojem her profesionálnì zabývá. Pøed zveøejnìním hry probìhl také dotazníkový prùzkum, který ukázal, ¾e hráèi ovládací metodu i samotnou hru hodnotí velmi pozitivnì.

Po získání a zpracování zpìtné vazby byla hra publikována na internetový obchod Google Play pod názvem Air Racer\footnote{\url{https://play.google.com/store/apps/details?id=cz.davidsabata.airracer}}. Tam se v~dobì dokonèování této práce umístila na pozici 9. nejlep¹í bezplatné aplikace v~kategorii závodních her a nainstalovalo si ji více ne¾ 70000 hráèù. Hra je kompatibilní s~více ne¾ 94\% mobilních zaøízení platformy Android (dle døíve uvedených statistik).
